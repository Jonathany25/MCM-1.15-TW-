...
public class LindsayChestContainer extends Container
{
    public final LindsayChestTileEntity tileEntity;
    private final IWorldPosCallable canInteractWitCallable;

    public LindsayChestContainer(final int windowId, final PlayerInventory playerInventory, final LindsayChestTileEntity tileEntity)
    {
        super(ModContainerTypes.LINDSAY_CHEST.get(), windowId);
    ...
    private static LindsayChestTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null");
        Objects.requireNonNull(data, "data cannot be null");
        final TileEntity tileatPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileatPos instanceof LindsayChestTileEntity) {
            return (LindsayChestTileEntity) tileatPos;

        }
        throw new IllegalStateException("Tile Entity is not correct!" + tileatPos);
    }

    public LindsayChestContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data)
    {
    ...
    {
        return isWithinUsableDistance(canInteractWitCallable,playerIn, BlockInit.LINDSAY_CHEST.get());
    }
